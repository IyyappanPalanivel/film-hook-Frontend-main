...
const NepalSub = () => {
  ...
  const [select, setSelect] = useState(nepal)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      ...
      else if (item.industry == 'LOLLYWOOD') {
        return (
          navigation.navigate("IndustryScreenOthers")
        )
      }
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={nepal.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
