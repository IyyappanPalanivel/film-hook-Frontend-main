...
const BangladeshSub = () => {
  ...
  const [select, setSelect] = useState(bangladesh)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'DHOLLYWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={bangladesh.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
