...
const TanzaniaSub = () => {
  ...
  const [select, setSelect] = useState(tanzania)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'SWAHILIWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={tanzania.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
