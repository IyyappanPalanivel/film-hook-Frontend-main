...
const PeruSub = () => {
  ...
  const [select, setSelect] = useState(peru)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'CHOLLYWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={peru.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
