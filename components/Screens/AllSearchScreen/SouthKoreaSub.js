...
const SouthKoreaSub = () => {
  ...
  const [select, setSelect] = useState(southKorea)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'HALLYUWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={southKorea.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
