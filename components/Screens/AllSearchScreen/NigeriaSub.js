...
const NigeriaSub = () => {
  ...
  const [select, setSelect] = useState(nigeria)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'NOLLYWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={nigeria.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
