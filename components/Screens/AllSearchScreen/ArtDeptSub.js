...
const ArtDeptSub=()=> {
    ...
    const jobPosts = [
        ...
        {
            id: '3',
            profession: 'ASSOCIATE ART DIRECTOR',
        },
    ...
    const handleOnPress = (item) => {
        const newItem = select.map((value) => {
            if (item.profession == 'ART DIRECTOR') {
                return (
                    navigation.navigate("Director")
                )
            }
            if (item.profession == 'ASSISTANT ART DIRECTOR') {
                return (
                    navigation.navigate("Director")
                )
            }
            if (item.profession == 'ASSOCIATE ART DIRECTOR') {
                return (
                    navigation.navigate("Director")
                )
            }
