...
const SomaliaSub = () => {
  ...
  const [select, setSelect] = useState(somalia)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'SOMALIWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={somalia.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
