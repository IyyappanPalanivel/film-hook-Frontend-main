...
const GhanaSub = () => {
  ...
  const [select, setSelect] = useState(ghana)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'GOLLYWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={ghana.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
