...
const RentalProducts = () => {
  ...
  const [cartRentalItems, setCartRentalItems] = useState([]);
  ...
  const addToCart = (productId) => {
    ...
    if (selectedProduct) {
      ...
      const existingCartItem = cartRentalItems.find((item) => item.id === productId);

      if (existingCartItem) {
        // Increase quantity by 1 if the item is already in the cart
        const updatedCart = cartRentalItems.map((item) =>
          item.id === productId ? { ...item, quantity: item.quantity + 1 } : item
        );
        setCartRentalItems(updatedCart);
      } else {
        // Add the selected product to the cartItems state with default quantity 1
        setCartRentalItems([...cartRentalItems, { ...selectedProduct, quantity: 1 }]);
      }
  ...
  const renderItem = ({ item }) => (
    // <View style={{ flexDirection: 'row', padding: 2, borderBottomWidth: 1, borderColor: '#ddd' }}>
    //   {/* Product Details */}
    ...
    //       <Image source={{ uri: item.data.CompanyLogo }} style={{ width:responsiveWidth(38), height:responsiveHeight(20), marginRight: 10,backgroundColor:"red" }} />
    //       <Text style={{ fontSize: responsiveFontSize(2), fontWeight: 'bold', alignSelf: 'center', color: 'black' }}>{item.data.Company_Name}</Text>
    //     </View>

    //     {/* {item.data.Company_Name} if need this you will add this insite text */}

    //     {/* <Text style={{ fontSize: responsiveFontSize(2), fontWeight: 'bold', color: 'black' }}>{item.data.Product_Name}</Text>
    //     <View style={{ height: responsiveHeight(10) }}>
    //       <Text style={{ fontSize: responsiveFontSize(1.9), lineHeight: responsiveHeight(3) }}>Condition: {item.data.ProductCondition} {'\n'}{item.data.Product_Description}{'\n'}Price per Day: INR {item.data.Price}</Text>
    ...
    <View style={{
      ...
      <View >
        ...
        <TouchableOpacity onPress={() => navigation.navigate('CartRental', { cartRentalItems })} style={{ width: responsiveWidth(9), height: responsiveHeight(4), left: responsiveWidth(80), bottom: responsiveHeight(3.8), }}>
          <Image source={require('../../Assets/AllSearch_Icon_And_Fonts/add-to-cart.png')} style={{
            width: 32, height: 32
      ...
      <ScrollView>
        ...
        <View style={{ flexDirection: 'row', padding: 2, borderColor: '#ddd', backgroundColor: 'rgba(189, 186, 187, 0.8)', padding: responsiveWidth(2), height: responsiveHeight(25), width: responsiveWidth(90), borderRadius: responsiveWidth(5), flexWrap: 'wrap', borderWidth: responsiveWidth(0.2), borderColor: 'black',marginBottom:5 }}>
          ...
          <TouchableOpacity style={{
            width: responsiveWidth(13), borderWidth: 1, backgroundColor: 'black', position: 'absolute', top: responsiveHeight(100), right: 40, borderRadius: 4
          }}>
