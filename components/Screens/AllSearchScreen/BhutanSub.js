...
const BhutanSub = () => {
  ...
  const [select, setSelect] = useState(bhutan)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'DZONGKHA MOVIES') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={bhutan.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
