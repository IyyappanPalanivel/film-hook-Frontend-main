...
const ChoreographySub=()=> {
    const navigation = useNavigation();
    const jobPosts = [
        {
            id: 1,
            profession: 'DANCER MASTER',
        },
        {
            id: '2',
            profession: 'BACKGROUND DANCER',
        },
    ...
    const handleOnPress = (item) => {
        const newItem = select.map((value) => {
            if (item.profession == 'DANCER MASTER') {
                return (
                    navigation.navigate("Director")
                )
            }
            if (item.profession == 'BACKGROUND DANCER') {
                return (
                    navigation.navigate("Director")
                )
            }
...
export default ChoreographySub;
