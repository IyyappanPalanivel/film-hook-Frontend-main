...
const ZimbabweSub = () => {
  ...
  const [select, setSelect] = useState(zimbabwe)
  const handleOnPress = (item) => {
    const newItem = select.map((value) => {
      if (item.industry == 'ZOLLYWOOD') {
        return (
    ...
    <View style={styles.container}>
      ...
      <View style={styles.flatListContainer} >
        ...
        <FlatList
          ...
          data={zimbabwe.filter((item) => ((item.industry).toLocaleLowerCase()).includes(search.toLocaleLowerCase()))}
