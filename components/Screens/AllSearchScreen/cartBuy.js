...
const CartBuy = ({ route, navigation }) => {
  const { BuycartItems } = route.params;

  const [items, setItems] = useState(BuycartItems || []);
  ...
  const increaseQuantity = (productId) => {
    const updatedItems = items.map((item) => {
      if (item && item.id === productId) {
        ...
        const updatedBuycartItems = items.map((cartItem) => (cartItem.id === productId ? updatedItem : cartItem));
        AsyncStorage.setItem('BuycartItems', JSON.stringify(updatedBuycartItems))
          .then(() => setItems(updatedBuycartItems))
  ...
  const checkout = () => {
    const totalBuyCost = items.reduce(
    ...
    AsyncStorage.removeItem('BuycartItems')
      .then(() => {
        ...
        navigation.navigate('CheckOutBuy', { BuycartItems: items, totalBuyCost }); // Navigate to Checkout screen
      })
      .catch((error) => console.error('Error clearing cart:', error));
  };
  useFocusEffect(
    React.useCallback(() => {
      const updateBuycartItems = async () => {
        try {
          const storedBuycartItems = await AsyncStorage.getItem('BuycartItems');
          if (storedBuycartItems !== null) {
            setItems(JSON.parse(storedBuycartItems));
          }
        } catch (error) {
