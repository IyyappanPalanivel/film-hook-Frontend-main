...
const WriterSub=()=> {
    ...
    const jobPosts = [
        ...
        {
            id: '2',
            profession: 'SCREEN PLAY WRITER',
        },
        {
            id: '3',
            profession: 'DIALOG WRITER',
        },
    ...
    const handleOnPress = (item) => {
        const newItem = select.map((value) => {
            if (item.profession == 'WRITER') {
                return (
                    navigation.navigate("Director")
                )
            }
            if (item.profession == 'SCREEN PLAY WRITER') {
                return (
                    navigation.navigate("Director")
                )
            }
            if (item.profession == 'DIALOG WRITER') {
                return (
                    navigation.navigate("Director")
                )
            }
            
            if (item.profession == 'STORY WRITER') {
                return (
                    navigation.navigate("Director")
                )
            }
            if (item.profession == 'SCRIPT ASSOCIATE') {
                return (
                    navigation.navigate("Director")
                )
            }
